...
class WordSwapMaskedLM_zl(WordSwap):
        ...
        **kwargs,
        ...
        if isinstance(masked_language_model, str):
            ...
            if os.path.exists(masked_language_model_cache):  # 如果是本地路径
                ...
                self._language_model = AutoModelForMaskedLM.from_pretrained(TRAIN_MODEL,output_attentions=True)
    ...
    def _bert_attack_replacement_words(self, current_text, index, id_preds, masked_lm_logits):
        return self._bert_attack_replacement_words_with_logits(current_text, index, id_preds, masked_lm_logits)
    ...
    def _get_replacement_words_with_logits(self, current_text, index, id_preds, masked_lm_logits):
        """使用logits优化的替换词选择"""
        # 初始化优化器参数
        epsilon = torch.nn.Parameter(torch.zeros_like(masked_lm_logits))
        ...
        for iter in range(self.num_iterations):
            ...
            # 计算当前logits并应用温度缩放
            current_logits = (masked_lm_logits + epsilon) / self.temperature
            ...
            diff_loss = torch.nn.functional.cosine_similarity(current_probs, original_onehot, dim=-1).mean()
    ...
    def _bert_attack_replacement_words_with_logits(self, current_text, index, id_preds, masked_lm_logits):
