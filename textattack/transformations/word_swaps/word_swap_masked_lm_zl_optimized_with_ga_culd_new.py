...
class WordSwapMaskedLM_zl(WordSwap):
    ...
    def _attack_replacement_phrases_with_logits(self, current_text, start_idx, end_idx, id_preds, masked_lm_logits):
        ...
        original_tensor = torch.tensor(original_ids, device=utils.device)
    ...
    def _attack_replacement_words_with_logits(self, current_text, index, id_preds, masked_lm_logits):
        ...
        # 使用遗传算法获取初始候选词
        ga_replacements = self._ga_replacement(current_text, index,index, id_preds, masked_lm_logits)
        ...
        # 合并两种方法的结果并去重
        all_replacements = list(set(ga_replacements + logits_replacements))
    ...
    def _get_replacement_words_with_logits(self, current_text, index, id_preds, masked_lm_logits):
        ...
        # 检查 masked_lm_logits 是否包含 NaN 或 inf
        assert torch.all(torch.isfinite(masked_lm_logits)), "masked_lm_logits contains NaN or inf"
        assert torch.all(torch.isfinite(epsilon)), "epsilon contains NaN or inf"
        ...
        # Debug 输出
        #print(f"original_word: {original_word}, type: {type(original_word)}")
        #print(f"original_ids: {original_ids}, type: {type(original_ids)}")
        #print(f"original_tensor: {original_tensor}, type: {type(original_tensor)}")
        ...
        while not found_valid_replacement and iter < self.num_iterations:
            ...
            current_logits = torch.clamp(current_logits, min=-1e2, max=1e2)  # 限制logits范围
            current_probs = torch.nn.functional.softmax(current_logits, dim=-1).detach()
            current_probs.requires_grad = True  # 创建一个新的张量并设置 requires_grad=True
            # 检查 current_logits 是否包含 NaN 或 inf
            assert torch.all(torch.isfinite(current_logits)), "current_logits contains NaN or inf"
            

            # Debug 输出
            #print(f"current_logits: {current_logits}, type: {type(current_logits)}")
            #print(f"current_probs: {current_probs}, type: {type(current_probs)}")
            ...
            # 计算与原词的差异损失
            # 获取原始词的所有 token 的 one-hot 编码
            original_onehot_list = []
            for token_id in original_tensor:
                token_onehot = torch.nn.functional.one_hot(token_id, num_classes=self._lm_tokenizer.vocab_size).float()
                original_onehot_list.append(token_onehot)

            # 将所有 token 的 one-hot 编码平均
            original_onehot = torch.stack(original_onehot_list).mean(dim=0)
            original_onehot = original_onehot.to(current_probs.device)  # 确保在同一设备上
            # 限制 one-hot 编码的范围
            original_onehot = torch.clamp(original_onehot, min=1e-6, max=1 - 1e-6)  # 防止极小的概率值
            diff_loss = torch.nn.functional.cosine_similarity(current_probs+ 1e-8, original_onehot+ 1e-8, dim=-1).mean()
            #print(f"original_onehot: {original_onehot}, type: {type(original_onehot)}")
            #print(f"current_probs: {current_probs}, type: {type(current_probs)}")
            # Debug 输出
            #print(f"flu_loss: {flu_loss}, type: {type(flu_loss)}")
            #print(f"diff_loss: {diff_loss}, type: {type(diff_loss)}")
            ...
            if loss < best_loss:
                ...
                # Debug 输出
                #print(f"top_values: {top_values}, type: {type(top_values)}")
                #print(f"top_indices: {top_indices}, type: {type(top_indices)}")
