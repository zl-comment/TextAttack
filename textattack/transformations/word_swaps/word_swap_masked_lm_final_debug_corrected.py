...
# Enhancements and fixes based on debugging suggestions.

class WordSwapMaskedLM_zl(WordSwap):
    def _bert_attack_replacement_phrases(
        self, current_text, start_idx, end_idx, id_preds, masked_lm_logits
    ):
        try:
            # Step 1: Replace phrase with [MASK]
            masked_text = current_text.replace_phrase_at_index(
                range(start_idx, end_idx),
                [self._lm_tokenizer.mask_token] * (end_idx - start_idx)
            )
            print(f"DEBUG: Masked text: {masked_text.text}")

            # Step 2: Encode the masked text
            current_inputs = self._encode_text(masked_text.text)
            current_ids = current_inputs["input_ids"].tolist()[0]
            print(f"DEBUG: Input IDs after masking: {current_ids}")

            try:
                masked_index = current_ids.index(self._lm_tokenizer.mask_token_id)
                print(f"DEBUG: MASK token index: {masked_index}")
            except ValueError:
                print("ERROR: MASK token not found!")
                return []

            # Step 3: Generate replacement candidates
            target_ids_pos = list(range(masked_index, masked_index + (end_idx - start_idx)))
            print(f"DEBUG: Target indices: {target_ids_pos}")

            top_preds = [id_preds[i] for i in target_ids_pos]
            print(f"DEBUG: Top predictions for masked positions: {top_preds}")

            products = itertools.product(*top_preds)
            replacement_phrases = []
            for bpe_tokens in products:
                candidate_phrase = "".join(
                    self._lm_tokenizer.convert_ids_to_tokens(bpe_tokens)
                ).replace("##", "")
                if not candidate_phrase.strip():
                    continue  # Skip empty candidates
                replacement_phrases.append(candidate_phrase)

            print(f"DEBUG: Candidate replacement phrases: {replacement_phrases}")
            return replacement_phrases

        except Exception as e:
            print("ERROR: Exception occurred during phrase replacement")
            traceback.print_exc()
            return []
